...
function create_thumbnail_mpv(file_path, timestamp, size, output_path, options)
	...
	local mpv_command = skip_nil({
		...
		(options.relative_scale and ("--vf=scale=iw*%d:ih*%d"):format(size.w, size.h) or ("--vf=scale=%d:%d"):format(
			size.w,
			size.h
		)),
...
function create_thumbnail_ffmpeg(file_path, timestamp, size, output_path, options)
	...
	local ffmpeg_command = {
		...
		"-vf",
		(options.relative_scale and ("scale=iw*%d:ih*%d"):format(size.w, size.h) or ("scale=%d:%d"):format(
			size.w,
			size.h
		)),
