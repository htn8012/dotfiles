...
function isempty(v)
	return not v or (v == "") or (v == 0) or (type(v) == "table" and not next(v))
end
...
function Thumbnailer:check_storyboard_async(callback)
	if thumbnailer_options.storyboard_enable and self.state.is_remote then
		...
		local sb_cmd = {
			...
			"--no-playlist",
			"--extractor-args",
			"youtube:skip=hls,dash,translated_subs", -- yt speedup
			"--",
			mp.get_property_native("path"),
		}

		mp.command_native_async(
			{ name = "subprocess", args = sb_cmd, capture_stdout = true },
			function(success, sb_json)
				if success and sb_json.status == 0 then
					...
					if sb and sb.duration and sb.width and sb.height and sb.fragments and #sb.fragments > 0 then
						...
						if sb.fps then
							...
							self.state.thumbnail_delta = sb.fragments[1].duration
								/ (self.state.storyboard.rows * self.state.storyboard.cols)
						...
						if thumbnailer_options.storyboard_max_thumbnail_count then
							divisor = math.ceil(
								self.state.thumbnail_count / thumbnailer_options.storyboard_max_thumbnail_count
							)
						end
...
function render()
	...
	if state.mp_screen_sizeX ~= current_screen_sizeX or state.mp_screen_sizeY ~= current_screen_sizeY then
		request_init_resize()
